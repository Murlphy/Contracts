...
public class SQLiteHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table "+DBUtils.DATABASE_TABLE+"("+DBUtils.NOTEPAD_ID+
                ...
                " text," +DBUtils.Mobile+" text,"+ DBUtils.NOTEPAD_TIME+ " text)");
    }
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {}
    //添加数据
    public boolean insertData(String userContent,String userMoblie,String userTime){
        ContentValues contentValues=new ContentValues();
        contentValues.put(DBUtils.NOTEPAD_CONTENT,userContent);
        contentValues.put(DBUtils.Mobile,userMoblie);
    ...
    //修改数据
    public boolean updateData(String id,String content,String mobilecontent,String userYear){
        ContentValues contentValues=new ContentValues();
        contentValues.put(DBUtils.NOTEPAD_CONTENT,content);
        contentValues.put(DBUtils.Mobile,mobilecontent);
    ...
    public List<NotepadBean> query(){
        ...
        if (cursor!=null){
            while (cursor.moveToNext()){
                ...
                String mcontent = cursor.getString(cursor.getColumnIndex(DBUtils.Mobile));
                String time = cursor.getString(cursor.getColumnIndex
                        (DBUtils.NOTEPAD_TIME));
                noteInfo.setId(id);
                noteInfo.setNotepadContent(content);
                noteInfo.setMoblieContent(mcontent);
